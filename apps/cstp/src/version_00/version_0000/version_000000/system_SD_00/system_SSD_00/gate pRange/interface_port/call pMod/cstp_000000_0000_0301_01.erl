...
%% @end
%%%-------------------------------------------------------------------
-module(cstp_000000_0000_0301_01).
...
processor_ref0({compute, STATUS}, STATE)->
    Interface =  whereis(interface),
    Edges = maps:get(edges, STATE),
    case maps:get(caller, STATE) of
        PID when PID == Interface->
            processor_ref0({interface, STATUS}, STATE);
        PID ->
            case lists:keyfind(PID, 1, Edges) of
                {Edge, Old_STATUS} ->
                    processor_ref0({edges, {Edge, Old_STATUS, STATUS}}, STATE);
                false ->
                    ft(_Issuer={?MODULE, processor_ref0}, _Fault={error, [permission]})
            end
    end;
processor_ref0({interface, STATUS}, STATE)->
    io:format("interfacePort_call, interface, status: ~p~n", [STATUS]),
    {ok, _reply=ok, STATE};
processor_ref0({edges, {Edge, Old_STATUS, New_STATUS}}, STATE)->
    io:format("interfacePort_call, edge: ~p~n", [{Edge, Old_STATUS, New_STATUS}]),
    {ok, _reply=ok, STATE};
%%
