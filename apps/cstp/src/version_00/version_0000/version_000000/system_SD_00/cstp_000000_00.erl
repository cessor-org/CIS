...
%%
compose({[_service={0,0} | Order], TX}, STATE) ->
    cstp_000000_0000:compose({Order, <<TX/binary, 0>>}, STATE);
...
%%      Service Sub-Domain 0 is reserved for system services
%%      Service Sub-Domain 255 is reserved for Sub-Domain extension
%%
compute(<<Service_SubDomain, TX_/binary>>, STATE)
  when Service_SubDomain == 0 ->
    cstp_000000_0000:compute(TX_, STATE);

compute(<<Service_SubDomain, _TX_/binary>>, _STATE)
  when Service_SubDomain == 255 ->
    %%cstp_000000_00FF:compute(TX_, STATE);
    ok;
